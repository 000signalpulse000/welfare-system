...
class ServiceMeeting(Base):
    ...
    user_id: Mapped[str] = mapped_column(String(32), index=True, nullable=False)
    service_type: Mapped[str] = mapped_column(String(64), nullable=False)
    meeting_title: Mapped[str | None] = mapped_column(String(128), nullable=True)
    ...
    user_attended: Mapped[bool | None] = mapped_column(nullable=True)
    agenda: Mapped[str | None] = mapped_column(Text, nullable=True)
    status_check: Mapped[str | None] = mapped_column(Text, nullable=True)
    ...
    next_policy: Mapped[str | None] = mapped_column(Text, nullable=True)
    next_action: Mapped[str | None] = mapped_column(Text, nullable=True)
    next_scheduled_date: Mapped[date | None] = mapped_column(Date, nullable=True)
    note: Mapped[str | None] = mapped_column(Text, nullable=True)
    user_signature: Mapped[str | None] = mapped_column(String(128), nullable=True)
    user_seal: Mapped[str | None] = mapped_column(String(255), nullable=True)
    staff_signature: Mapped[str | None] = mapped_column(String(128), nullable=True)
    staff_seal: Mapped[str | None] = mapped_column(String(255), nullable=True)
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True),
        server_default=func.now(),
        nullable=False,
